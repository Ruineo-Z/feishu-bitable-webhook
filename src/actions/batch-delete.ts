...
const batchDelete: ActionHandler = {
  async execute(params: ActionParams, context: Record<string, unknown>): Promise<ActionResult> {
    ...
    const log = traceId ? createLoggerWithTrace(traceId, 'batch-delete.ts') : logger
    ...
    log.info('[batch-delete] 请求参数:', JSON.stringify({ app_token, table_id, recordsCount: record_ids.length }, null, 2))
    ...
    try {
      await client.bitable.v1.appTableRecord.batchDelete({
        ...
        } as unknown as { records: string[] }
      })

      log.info('[batch-delete] 成功删除:', record_ids.length, '条记录')
      ...
      log.error('[batch-delete] 错误:', error?.response?.data || error)
