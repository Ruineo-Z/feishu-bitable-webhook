...
The workflow management UI SHALL allow users to create workflows via `POST /api/workflows` and MUST provide node-centric visual authoring with table scope validation, optional event filters, branching DSL structure, and source-aware condition fields before submission.

#### Scenario: Create workflow through node-centric visual editor
- **WHEN** a user creates a new workflow in visual mode
- **THEN** the UI MUST support configuring workflow metadata, selecting node types from add-node catalog, and submitting generated DSL payload through existing create API
...
The workflow management UI SHALL load workflow details from `GET /api/workflows/{id}` before editing and MUST render existing workflow as editable node graph while preserving source-aware condition fields and guard configuration when updating via `PUT /api/workflows/{id}`.
...
#### Scenario: Load existing workflow into node graph
- **WHEN** a user opens an existing workflow in editor
- **THEN** the UI MUST render nodes, branches, and connections from stored workflow definition and allow continued editing in visual mode
