# Spec: Workflow Management Web UI

## Purpose

Define the functional requirements of the workflow management UI so operators can create, update, review, and delete workflows with correct scope and branching semantics.
## Requirements
...
- **THEN** the UI MUST display workflow name, active status, scope type, event filter summary, and update time for each row
